...
- `.github/chart-fixtures/<chart>-deterministic.conf` lists the chart's `ci/` values files whose render must be byte-identical across two runs. `pleme-io/actions/helm-render-determinism` renders each twice and fails on any difference, so a path that is meant to be GitOps-stable cannot silently regain a `genCA`, `randAlphaNum`, `uuidv4` or `now` call. Any chart can opt in by adding the file; charts without one are skipped.
- `chart_test.yaml` is declarative on purpose: its steps are `uses:` with `with:`, and the work lives in shared actions (`helm-ct-list-changed`, `helm-unittest`, `helm-render-determinism`, `kube-tls-fixture`, `kube-namespace-diagnostics`, `helm-release-cleanup`), all open source under `pleme-io/actions` and SHA-pinned here like every other action. Add logic there rather than growing a `run:` block back; one-line invocations are the only shell this file should carry.
