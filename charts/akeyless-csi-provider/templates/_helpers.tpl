...
*/}}
{{- define "akeyless.pdb.maxUnavailable" -}}
...
template logic.
*/}}
{{- define "akeyless.mode" -}}
  {{- if .Values.injector.externalakeylessAddr -}}
    {{- $_ := set . "mode" "external" -}}
...
{{/*
Set's the args for custom command to render the akeyless configuration
file with IP addresses to make the out of box experience easier
for users looking to use this chart with Consul Helm.
*/}}
{{- define "akeyless.args" -}}
  {{ if or (eq .mode "standalone") (eq .mode "ha") }}
          - |
            cp /akeyless/config/extraconfig-from-values.hcl /tmp/storageconfig.hcl;
            ...
            /usr/local/bin/docker-entrypoint.sh akeyless server -config=/tmp/storageconfig.hcl {{ .Values.server.extraArgs }}
   {{ else if eq .mode "dev" }}
          - |
            /usr/local/bin/docker-entrypoint.sh akeyless server -dev {{ .Values.server.extraArgs }}
  {{ end }}
{{- end -}}
...
*/}}
{{- define "akeyless.envs" -}}
  {{ if eq .mode "dev" }}
            - name: akeyless_DEV_ROOT_TOKEN_ID
              value: {{ .Values.server.dev.devRootToken }}
            - name: akeyless_DEV_LISTEN_ADDRESS
              value: "[::]:8200"
  {{ end }}
{{- end -}}
...
{{- define "akeyless.mounts" -}}
  ...
  {{ if and (ne .mode "dev") (or (.Values.server.standalone.config)  (.Values.server.ha.config)) }}
            - name: config
              mountPath: /akeyless/config
  {{ end }}
  {{- range .Values.server.extraVolumes }}
            - name: userconfig-{{ .name }}
              readOnly: true
              mountPath: {{ .path | default "/akeyless/userconfig" }}/{{ .name }}
  {{- end }}
...
{{- define "akeyless.volumeclaims" -}}
  ...
  volumeClaimTemplates:
    ...
    - metadata:
        name: data
        {{- include "akeyless.dataVolumeClaim.annotations" . | nindent 6 }}
      spec:
        accessModes:
    ...
    - metadata:
        name: audit
        {{- include "akeyless.auditVolumeClaim.annotations" . | nindent 6 }}
      spec:
        accessModes:
...
*/}}
{{- define "akeyless.ui.annotations" -}}
...
*/}}
{{- define "akeyless.serviceAccount.name" -}}
{{- if .Values.server.serviceAccount.create -}}
    {{ default (include "akeyless.fullname" .) .Values.server.serviceAccount.name }}
{{- else -}}
...
*/}}
{{- define "akeyless.serviceAccount.annotations" -}}
...
*/}}
{{- define "akeyless.ingress.annotations" -}}
...
*/}}
{{- define "akeyless.route.annotations" -}}
...
{{/*
Sets extra akeyless server Service annotations
*/}}
{{- define "akeyless.service.annotations" -}}
...
*/}}
{{- define "akeyless.psp.annotations" -}}
...
*/}}
{{- define "akeyless.statefulSet.annotations" -}}
...
*/}}
{{- define "akeyless.dataVolumeClaim.annotations" -}}
...
*/}}
{{- define "akeyless.auditVolumeClaim.annotations" -}}
...
*/}}
{{- define "akeyless.extraEnvironmentVars" -}}
...
*/}}
{{- define "akeyless.extraSecretEnvironmentVars" -}}
