...
| `gatewayRootMode`                             | Force the gateway home directory: `true` = `/root`, `false` = `/home/akeyless`. Unset = auto-detect from a non-null `deployment.containerSecurityContext.runAsUser`, or a non-null `deployment.securityContext.runAsUser` when `deployment.securityContext.enabled: true` (container wins over pod), else from `image.repository`/`image.tag`. `gatewayRootMode: true` combined with `akeylessStrictMode: true`, or with a non-root `runAsUser` from either securityContext, fails the render. | `nil`                    |
