...
| `gatewayRootMode`                             | Force the gateway home directory: `true` = `/root`, `false` = `/home/akeyless`. Unset = auto-detect from a non-null `deployment.containerSecurityContext.runAsUser` or `deployment.securityContext.runAsUser` (container wins over pod), else from `image.repository`/`image.tag`. `gatewayRootMode: true` with `akeylessStrictMode: true` fails the render. | `nil`                    |
